...
#search { width: 142px; padding: 5px;  margin: 3px auto; border: 2px solid #ddd; border-radius: 8px; font-size: 20px;}

#searchBox { position: fixed; right: 28px; bottom: 37px; background: #ddd; padding: 5px;  border-radius: 3px; box-shadow: -1px -1px 6px #ccc;}

#search_folder { width: 142px; padding: 5px;  margin: 3px auto; border: 2px solid #ddd; border-radius: 8px; font-size: 20px;}

#searchBoxFolder { position: fixed; right: 200px; bottom: 37px; background: #ddd; padding: 5px;  border-radius: 3px; box-shadow: -1px -1px 6px #ccc;}
...
<div id="container">
  ...
  <div id="searchBoxFolder">
	<input type="text" placeholder="Поиск папки" size="10" maxlength="10" id="search_folder" name="search"/></div>
	
	<div id="searchBox">
	<!--<label for="search" id="lblSearch"><?php echo $text_search; ?></label>-->
	<input type="text" size="3" maxlength="3" placeholder="Поиск файлов" id="search" name="search"/></div>
	</div>
...
$(document).ready(function() { 
	...
	/*Поиск папки*/
    $('input#search_folder').click(function() {
	var x = $('#column-left li#top li[directory]');
	x.each(function() {
...
$('input#search_folder').keyup(function(event) {
		$('input#search_folder').focus();
		var search_text = $('input#search_folder').val();
		var rg = new RegExp(search_text,'i');
		$('#column-left li#top li[directory]').each(function(){
            if($(this).text().match(rg)){
                $(this).show();
            }else{
                $(this).hide();
            };
