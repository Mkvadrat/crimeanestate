...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', clean = '" . (isset($data['clean']) ? (int)$data['clean'] : 0) . "', status = '" . (int)$data['status'] . "', image = '".$this->db->escape($data['image'])."', show_in_blog = '".$this->db->escape($data['show_in_blog'])."', blog_type = '".$this->db->escape($data['blog_type'])."', blog_category = '".$this->db->escape($data['blog_category'])."', date_added = '".$this->db->escape($date_added)."'");
	...
	public function editInformation($information_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', clean = '" . (isset($data['clean']) ? (int)$data['clean'] : 0) . "', status = '" . (int)$data['status'] . "', image = '".$this->db->escape($data['image'])."', show_in_blog = '".$this->db->escape($data['show_in_blog'])."', blog_type = '".$this->db->escape($data['blog_type'])."', blog_category = '".$this->db->escape($data['blog_category'])."' WHERE information_id = '" . (int)$information_id . "'");
