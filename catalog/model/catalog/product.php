...
class ModelCatalogProduct extends Model {
	...
	public function getProductAgent($user_id, $start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE agent = '" . (int)$user_id . "' AND status = 1 ORDER BY product_id ASC LIMIT " . (int)$start . "," . (int)$limit );
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_description JOIN " . DB_PREFIX . "product ON " . DB_PREFIX . "product.product_id = " . DB_PREFIX . "product_description.product_id AND " . DB_PREFIX . "product.agent = '" . (int)$user_id . "' AND product.status = 1 ORDER BY product.product_id ASC LIMIT " . (int)$start . "," . (int)$limit );
		
        return $query->rows;
    }
	public function getTotalProductAgent($user_id) {
        $query = $this->db->query("SELECT COUNT(product_id) FROM " . DB_PREFIX . "product WHERE agent = '" . (int)$user_id . "' AND status = 1");
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'this_lots'        => $query->row['this_lots'],
                'agent'            => $query->row['agent'],
				'sku'              => $query->row['sku'],
				'upc'              => $query->row['upc'],
				'ean'              => $query->row['ean'],
				'lat_lng'          => $query->row['lat_lng'],
				'price_from'       => $query->row['price_from'],
				'jan'              => $query->row['jan'],
				'isbn'             => $query->row['isbn'],
				'mpn'              => $query->row['mpn'],
	...
	public function getProductsCompany($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		if (!$product_data) {
			...
			$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.company_value = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'"; 
	...
	public function getTotalProductsCompany($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
	...
	//Вывод некоторых опций (с $Required = 1) только разрешенных на вывод
	public function GetRequiredOptions($product_id){
				
		/*Неразрешенные опции*/
        $query = $this->db->query("SELECT DISTINCT option_description.name, product_option.option_value, option.type FROM ".DB_PREFIX."`option_description` JOIN ".DB_PREFIX."`product_option` JOIN ".DB_PREFIX."`option` ON product_option.product_id = '" . (int)$product_id . "' AND option_description.option_id = product_option.option_id AND option.option_id = product_option.option_id AND product_option.required = 1 AND option_description.option_id NOT IN (101, 14, 19, 15, 16, 17) ORDER BY product_option.product_sort_option_id ASC");
		/*Неразрешенные опции*/
	
		return $query->rows;	
	}
	
	public function getPriceFromСomparison($product_id) {
