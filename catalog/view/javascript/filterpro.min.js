...
function doFilter(b) {
    ...
    valuta = jQuery('#valutesort').attr('data-valute');
...
jQuery(document).ready(function () {
    ...
    /*jQuery(".pagination ").on("click",".links a", (function (e) {
        var hash = window.location.hash.substr(1);
        ...
        doFilter(false);
        jQuery('html, body').animate({ scrollTop: jQuery('.product-list').offset().top }, 'slow');
        return false;
    }));*/
...
// deserialize
    ...
    jQuery("#filterpro").deserialize(hash);
    if (hash) {
        var valuta = jQuery( "input:radio[name=valute]:checked" ).val();
    ...
    jQuery("#filterpro img").bind("click", function() {
    ...
    if (jQuery(".sort select").length) {
        if (jQuery("#filterpro_sort").val()) {
            jQuery(".sort select").each(function(i, e) {
                if (gUV(jQuery(this).val(), "sort") == jQuery("#filterpro_sort").val() && gUV(jQuery(this).val(), "order") == jQuery("#filterpro_order").val()) {
            ...
            jQuery("#filterpro_sort").val(gUV($val, "sort"));
            jQuery("#filterpro_order").val(gUV($val, "order"))
        }
    }
    if (jQuery("#filterpro_limit").length) {
        if (jQuery("#filterpro_limit").val()) {
            jQuery(".limit select").val(jQuery("#filterpro_limit").val());
        } else {
            jQuery("#filterpro_limit").val(gUV(jQuery(".limit select").val(), "limit"));
        }
    }
    doFilter(true)
});
