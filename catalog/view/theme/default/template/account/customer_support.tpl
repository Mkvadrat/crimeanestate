...
<script type="text/javascript">
	jQuery("#customer_support_1st_category_id").change(function(){
		jQuery('#customer_support_2nd_category_id').load('index.php?route=account/customer_support/get_2nd_category&1st_category=' + $("#customer_support_1st_category_id").val());	
	});
	...
	function fn_submit_new_enquiry(frm_id){
		jQuery.ajax({
			...
			beforeSend: function() {
				jQuery('.success, .warning').remove();
				jQuery('#button_submit', '#'+frm_id).attr('disabled', 'disabled');
				jQuery('#'+frm_id).before('<div class="attention"><img src="catalog/view/theme/default/image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
			},
			complete: function() {
				jQuery('#button_submit', '#'+frm_id).attr('disabled', '');
				jQuery('.attention').remove();
			},
			success: function(data) {
				if (data.error) {
					jQuery('#'+frm_id).before('<div class="warning">' + data.error + '</div>');
				}
				
				if(data.success) {
					jQuery('#'+frm_id).before('<div class="success">' + data.success + '</div>');
					
					jQuery('#subject', '#'+frm_id).val('');
					jQuery('#enquiry', '#'+frm_id).val('');
					alert(data.success);
					location.reload();
				}
			}
	...
	function fn_delete_enquiry(enquiry_id){
		...
		jQuery.ajax({
			...
			beforeSend: function() {
				jQuery('.success, .warning').remove();
				jQuery('#button_delete_' + enquiry_id).attr('disabled', 'disabled');
				jQuery('#enquiry-content-' + enquiry_id).before('<div class="attention"><img src="catalog/view/theme/default/image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
			},
			complete: function() {
				jQuery('#button_delete_' + enquiry_id).attr('disabled', '');
				jQuery('.attention').remove();
			},
			success: function(data) {
				if (data.error) {
					jQuery('#enquiry-content-'+enquiry_id).before('<div class="warning">' + data.error + '</div>');
				}
				
				if(data.success) {
					jQuery('#new-enquiry').before('<div class="success">' + data.success + '</div>');
					jQuery('#enquiry-' + enquiry_id).remove();
					jQuery('#enquiry-detail-' + enquiry_id).remove();
				}
			}
		});
	}	
